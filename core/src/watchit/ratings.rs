...
pub fn merge_ratings(sets: Vec<Vec<Rating>>) -> Vec<Rating> {
    ...
    for set in sets {
        for r in set {
            let key = match by_id.contains_key(&r.id) {
                true => r.id.clone(),
                false if !r.title.is_empty() => by_title
                    .get(&title_key(&r.title, r.year))
                    .cloned()
            ...
            if newer {
                if !r.title.is_empty() {
                    by_title.insert(title_key(&r.title, r.year), key.clone());
                }
                by_id.insert(key, r);
            }
        }
...
pub fn rating_for(ratings: Vec<Rating>, id: String, title: String, year: i32) -> i32 {
    ...
    let want = title_key(&title, year);
    ratings.iter()
        .find(|r| !r.title.is_empty() && title_key(&r.title, r.year) == want)
        .map(|r| r.score)
        .unwrap_or(0)
}

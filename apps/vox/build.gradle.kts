...
// Identical pattern to apps/tasks: cargo-ndk builds the per-ABI .so,
// uniffi-bindgen emits the Kotlin bindings. vox uses the core only for the
// hyperlist parse/serialize/add (so a "→ Tasks" capture stays byte-compatible
// with todo.hl). Keep PATH cleaned (`/usr/bin:$PATH`) — ~/bin/cc shadows the
// real C compiler the Rust build scripts call.
...
dependencies {
    ...
    // Whisper transcription HTTP.
    implementation(libs.okhttp)

    // JNA backs the generated UniFFI bindings.
